...
const add_new_course = function () {
  ...
  const trig_add_lecture = function () {
    ...
    $('#add_lecture').off('click').on('click', function (e) {
      ...
      kanban_add_lecture(section_id,lecture_id,lecture_name);
...
//return false;
  ...
  const trig_add_yt_list = function () {
    $('#add_yt_list').on('click',function (e) {
      e.preventDefault();
      const list_link = $('#list_link').val();
      $.ajax({
        type: 'POST',
        url: '/aajax/search_yt_list',
        dataType: 'json',
        data:{
         list_link:list_link
        },
  ...
  const render_yt_list= function(data){
    for(let i = 0; i < data.length; i++){
      let row =  data[i];
      let lecture_name = row.snippet.title;
      let section_id = $('#section_selector').val();
      //let lecture_id = '_' + lecture_name;
      let lecture_id = random_id(10);


      kanban_add_lecture_yt_list(section_id,lecture_id,lecture_name,row);
  ...
  const kanban_add_lecture_yt_list = function(section_id,lecture_id,lecture_name,row){
    kanban4.addElement(
      section_id,
      {
        'id': lecture_id,
        'title':
          ...
          '<div class="kt-user-card-v2 kt-pt15"><div class="kt-user-card-v2__pic2" style="width: 120px">'+
          '<a href="" data-media_url="' + row.snippet.resourceId.videoId + '" data-type="video/youtube" class="media_info" data-toggle="modal" data-target="#kt_modal_6">' +
          '<img height="50" src="' + row.snippet.thumbnails.medium.url + '" style="border-radius: 4px;"/>' +
          '</a>'+
          '</div>' +
          '<div class="kt-user-card-v2__details"><a class="kt-user-card-v2__name" href="#">' + row.snippet.title + '</a>' +
          '<span class="kt-user-card-v2__desc" data-duration_seconds="' + row.duration_seconds + '" data-duration_string="' + row.duration_string + '">Duration: ' + row.duration_string + '</span>' +
          '</div></div>'+
